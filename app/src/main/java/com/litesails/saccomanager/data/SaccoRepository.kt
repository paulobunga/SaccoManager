...
class SaccoRepository(private val context: Context, private val db: SaccoDatabase) {
    ...
    /** Look up a [SaccoUser] by their Clerk user ID (used for session restoration on startup). */
    suspend fun getUserByClerkUserId(clerkUserId: String): SaccoUser? = withContext(Dispatchers.IO) {
        userDao.getUserByClerkUserId(clerkUserId)
    }
    ...
    suspend fun resetPassword(memberId: String): Pair<Boolean, String> = withContext(Dispatchers.IO) {
        val user = userDao.getUserById(memberId)
        if (user != null) {
            // Delegate password reset to Clerk (REQ-4, REQ-5).
            // No password is stored locally — Clerk owns all credentials.
            val resetResult = ClerkAuthManager.sendPasswordReset(user.email)
    ...
    suspend fun registerUser(user: SaccoUser, profile: MemberProfile, password: String = "") = withContext(Dispatchers.IO) {
        db.withTransaction {
            ...
            // Initial sync enqueue with empty clerkUserId — will be updated on first successful login
        ...
        // REQ-4, REQ-5: Register the user in Clerk so credentials are never stored locally.
        // This runs outside the Room transaction because Clerk is a network call.
        // If Clerk fails we still keep the local Room record — the user is registered
        // offline-first and their clerkUserId will be populated on next successful login.
        if (password.isNotEmpty()) {
            val authResult = ClerkAuthManager.register(user.email, password)
            authResult.fold(
                onSuccess = { clerkUser ->
                    // Store the Clerk user ID in Room so the local record is linked to Clerk.
                    val userWithUid = user.copy(clerkUserId = clerkUser.uid)
                    userDao.insertUser(userWithUid)
                    // Re-enqueue so the Supabase document also gets the clerkUserId field.
                    syncEngine.enqueue("USER_REGISTER", userWithUid, SaccoUser::class.java)
                    Log.d("SaccoRepository", "Clerk account created for ${user.email}, uid=${clerkUser.uid}")
                },
                onFailure = { error ->
                    // Degraded mode: the user is registered locally but has no Clerk account yet.
                    // They can still use the app offline; Clerk registration will be retried on login.
                    Log.e("SaccoRepository", "Clerk registration failed for ${user.email}: ${error.message}")
                }
            )
        }
    }
